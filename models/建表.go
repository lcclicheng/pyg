...
//建表
func Init() {
	orm.RegisterDataBase("default", "mysql", "root:123456@tcp/pyg")
	orm.RegisterModel(new(User), new(Address), new(TpshopCategory))
	orm.RegisterModel(new(Goods), new(GoodsSKU), new(GoodsType), new(GoodsImage), new(IndexGoodsBanner), new(IndexTypeGoodsBanner), new(IndexPromotionBanner))
	orm.RegisterModel(new(OrderInfo),new(OrderGoods))
